...
<body>
    <div class="EventBooking_details_box_all">
        <div class="animate__animated animate__fadeInUp">
            <a href="change_status_waterdelivery.php"><button type="button" class="page_button">Change Payment
                    Status</button></a>
        </div>
        <div class="animate__animated animate__fadeInUp">
            <div>
                <h2>
                    Booked Water Delivery Orders
                </h2>
            </div>
        </div>
        <?php
        ...
        $select_query = "SELECT * FROM WaterDelivery_details";
        ...
        if (mysqli_num_rows($data) > 0) {
            while ($row = mysqli_fetch_array($data)) {
                ...
                echo "<div><b>Date: </b>" . $row["CD"] . "</div>";
                echo "<div><b>Number of Bottles: </b>" . $row["BottlesNo"] . "</div>";
                echo "<div><b>Cost: </b>" . $row["cost"] . "</div>";
                if($row["status"] == 'T'){
                    $status = "Paid";
                }
